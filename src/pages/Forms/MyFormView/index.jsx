...
function MyFormView() {
  ...
  const handleDelete = async () => {
    if (confirm('Are you sure you want to delete this form?')) {
      try {
        await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/forms/delete`, {
  ...
  try {
    await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/forms/stop`, {
      formId: form.id,
    }, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
    });
    window.location.reload();
    toast.success('Form stopped successfully');
    
  } catch (err) {
    console.error('Failed to stop form', err);
    toast.error('Failed to stop form');
  }
};

const handleActivate = async () => {
  try {
    await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/forms/activate`, {
      formId: form.id,
    }, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
    });
...
const handleCopyLink = () => {
  const formLink = `${window.location.origin}/form/${form.id}`;
  ...
  return (
    <div className="bg-[#F4F7FC]">
      ...
      <div className="min-h-screen font-[poppins] px-4 md:px-10 pt-24">
        <h1 className="text-black text-xl md:text-2xl font-bold mb-6">
          <button
            onClick={() => navigate('/my-forms')}
            className=" text-blue-600 px-2 py-2   order-1 md:order-1 hover:text-blue-800"
            >
            <FaArrowLeft className="text-[20px] tex-bold" />
          </button>My Form View</h1>
          ...
          <div className="flex-1 flex flex-col gap-4 md:gap-6 w-full">
            <div className="flex flex-col md:flex-row justify-end gap-3">
              <button
                ...
                className=" text-blue-600  py-2   order-1 md:order-1 hover:text-blue-800"
              >
                <FiCopy className="text-[28px] tex-bold" />
              </button>
              <button
                onClick={handleDelete}
                className=" text-blue-600 px-2 py-2   order-1 md:order-1 hover:text-blue-800"
              >
                <FaTrash className="text-[20px] tex-bold" />
              </button>

              
              <button
                onClick={() => navigate(`/form-response/${form.id}`)}
                className="border border-blue-600 text-blue-600 px-4 py-2 rounded-md hover:bg-blue-50 order-3"
              >
                Response
              </button>
              ...
              {form.is_published ? (
                <button
                  onClick={handleStop}
                  className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 order-4"
                >
                  Stop
                </button>
              ) : (
                <button
                  ...
                  className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 order-3"
                >
                  Publish
                </button>
              )}
                ...
                {q.type === 'Checkbox' && (
                  <div className="flex flex-col flex-wrap">
                    {q.options.map((opt, i) => (
                      <label
                        key={i}
                        className="inline-flex items-center mr-4 mb-[8px] text-[18px] cursor-not-allowed"
                      >
                        <input
                          type="checkbox"
                          className="w-[20px] h-[20px] border-gray-300 rounded-md mr-[8px] cursor-not-allowed"
                          value={opt}
